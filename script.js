...
window.makeEditable = function (id) {
  ...
  fields.forEach((field) => {
    ...
    const handleBlur = () => {
      let newValue;
      if (field === "name" || field === "subject") {
        newValue = editableElement.value.trim();
        // Replace the input with its value as text
        element.innerHTML = newValue; // Put the text back into the td
      } else {
        // 'marks' field
        element.contentEditable = false; // Turn off contentEditable
        newValue = editableElement.textContent.trim();
      }
      ...
      if (newValue !== originalValue) {
        console.log(
          `Value changed for '${elementId}'. Updating field: ${field}, New Value: ${newValue}`
        );
        window.updateField(id, field, newValue); // Call global updateField
      } else {
        console.log(`Value not changed for '${elementId}'. No update needed.`);
      }
    };
